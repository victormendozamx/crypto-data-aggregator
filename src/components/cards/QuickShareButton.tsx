...
export default function QuickShareButton({ title, url, className = '' }: QuickShareButtonProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      ...
      {/* Tooltip */}
      {showTooltip && !copied && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-text-primary bg-surface rounded shadow-lg whitespace-nowrap border border-surface-border">
          Share
      ...
      {/* Copied confirmation */}
      {copied && (
        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-text-primary bg-gain rounded shadow-lg whitespace-nowrap">
          Copied!
          <div className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-gain" />
        </div>
      )}
    </div>
  );
