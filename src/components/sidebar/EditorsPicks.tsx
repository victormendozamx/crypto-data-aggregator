...
const defaultStyle = { dot: 'bg-neutral-400 dark:bg-neutral-500', bg: 'bg-neutral-100 dark:bg-neutral-800' };
  ...
  return (
    <div className="bg-neutral-50 dark:bg-neutral-900 rounded-2xl shadow-card dark:shadow-none dark:border dark:border-neutral-800 p-6">
      ...
      <div className="flex items-center gap-2 mb-5">
        <svg className="w-5 h-5 text-neutral-700 dark:text-neutral-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
        ...
        {picks.map((article, index) => {
          ...
          return (
            <Link
              ...
              className="group block p-4 -mx-4 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all duration-200 focus-ring"
              role="listitem"
            >
              {/* Source Badge */}
              <div className="flex items-center gap-2 mb-2">
                <span 
                  className={`w-2 h-2 rounded-full ${defaultStyle.dot}`} 
                  aria-hidden="true" 
                />
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${defaultStyle.bg} text-neutral-700 dark:text-neutral-300`}>
                  {article.source}
                </span>
                {index === 0 && (
