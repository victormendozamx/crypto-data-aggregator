  ...
  return (
    <div className="min-h-screen bg-white dark:bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {selectedCoins.length < MAX_COMPARE && (
              <div className="relative">
                ...
                {showCoinSelector && (
                  <div className="absolute top-full left-0 mt-2 w-72 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 shadow-xl z-10">
                    ...
                    <div className="max-h-60 overflow-y-auto p-2">
                      {filteredCoins.slice(0, 10).map(coin => (
                        ...
                        >
                          ...
                          <div>
                            <p className="font-medium text-neutral-900 dark:text-white text-sm">{coin.name}</p>
                            <p className="text-xs text-neutral-500 dark:text-neutral-400">{coin.symbol.toUpperCase()}</p>
                          </div>
                        </button>
                      ))}
        ...
        {/* Error */}
        {error && (
          <div className="mb-6 p-4 bg-neutral-100 dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 rounded-xl flex items-center gap-3 text-neutral-700 dark:text-neutral-300">
                    ...
                    return (
                      <div key={coinId} className="text-center">
                        ...
                        <p className={`text-xs font-medium ${
                          change >= 0 ? 'text-neutral-900 dark:text-white' : 'text-neutral-500 dark:text-neutral-400'
                        }`}>
            ...
            <table className="w-full">
              ...
              <tbody>
                ...
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">Price</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                    return (
                      <td key={coinId} className="px-4 py-4 text-right font-medium text-neutral-900 dark:text-white">
                        ${coin?.current_price.toLocaleString(undefined, { 
                          minimumFractionDigits: 2, 
                          maximumFractionDigits: coin?.current_price && coin.current_price < 1 ? 6 : 2 
                        }) || '-'}
                      </td>
                    );
                  })}
                ...
                {/* Market Cap */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">Market Cap</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                    const mcap = coin?.market_cap || 0;
                    return (
                      <td key={coinId} className="px-4 py-4 text-right text-neutral-600 dark:text-neutral-300">
                ...
                {/* 24h Change */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">24h Change</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                ...
                {/* 7d Change */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">7d Change</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                ...
                {/* ATH */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">All-Time High</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                    return (
                      <td key={coinId} className="px-4 py-4 text-right text-neutral-600 dark:text-neutral-300">
                ...
                {/* From ATH */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">From ATH</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                ...
                {/* Volume */}
                <tr className="border-b border-neutral-100 dark:border-neutral-800">
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">24h Volume</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                ...
                {/* Circulating Supply */}
                <tr>
                  <td className="px-6 py-4 text-sm text-neutral-600 dark:text-neutral-300">Circulating Supply</td>
                  {selectedCoins.map(coinId => {
                    const coin = coinData.get(coinId);
                    ...
                    return (
                      <td key={coinId} className="px-4 py-4 text-right text-neutral-600 dark:text-neutral-300">
                        {supply >= 1e9 ? `${(supply / 1e9).toFixed(2)}B` :
                         ...
                         supply.toLocaleString()} {coin?.symbol.toUpperCase()}
                      </td>
                    );
                  })}
                </tr>
