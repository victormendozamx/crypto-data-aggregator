        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {trending.slice(0, 7).map((coin, index) => (
            <Link
              key={coin.id}
              href={`/coin/${coin.id}`}
              className="flex-shrink-0 flex items-center gap-2 bg-neutral-50 dark:bg-neutral-800 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-lg px-3 py-2 transition-colors"
            >
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <h2 className="font-bold text-lg flex items-center gap-2">
              ...
              <span className="text-neutral-900 dark:text-white">Top {activeTab === 'gainers' ? 'Gainers' : 'Losers'}</span>
            </h2>
          </div>
        ...
        <div className="space-y-2">
          {mounted && displayList.map((coin) => (
            <Link
              key={coin.id}
              href={`/coin/${coin.id}`}
              className="flex items-center justify-between hover:bg-neutral-50 dark:hover:bg-neutral-800 rounded-lg px-2 py-1.5 transition-colors"
            >
              <div className="flex items-center gap-3">
                ...
                <span className="font-medium text-sm text-neutral-900 dark:text-white">{coin.symbol.toUpperCase()}</span>
